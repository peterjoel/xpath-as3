package memorphic.parser
{
	...
	public class Tokenizer
	{
		 ...
		 */
		public function skipAhead(numTokens:int):void
		{
			var n:int = numTokens;
			while(n--){
				nextToken();
		...
		public function lookAhead(offset:int=1):Token
		{
			...
			var cachedState:TokenizerState
			if((cachedState = stateCache[tokenIndex] as TokenizerState))
			{
		...
		public function getTokenMetrics(token:Token):TokenMetrics
		{
			...
			return new TokenMetrics(token.sourceIndex, linesToToken.length, column, token.length);
		}
		...
		{
			var cachedState:TokenizerState
			if(tokenPosition < stateCache.length 
					&& (cachedState = stateCache[tokenPosition] as TokenizerState))
			{
				return cachedState;
			}else{
